...
| =ONESHOT_VIM=             | Enables running vim in "oneshot" mode, see [[#oneshot-vim][oneshot vim]] for details.                                             | -540 B            |
...
exit the mode, same goes if you call any action like ~daw~, ~Y~ and the like.
